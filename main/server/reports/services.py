...
def _map_report_payload(data, fallback_status=None, image_name=None):
    item_name = data.get('item_name') or data.get('itemName')
    description = data.get('description')
    status = data.get('status') or fallback_status
    location = data.get('location')

    if not item_name or not status or not location:
        return None, "item_name/itemName, status, and location are required"
